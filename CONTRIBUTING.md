...
- Relevant log excerpts:
  ...
  - Output of `sudo /usr/local/bin/debsecan-filtered --test` (with sensitive data redacted)
...
3. **Test it.**
   ...
   - For filter changes: run `sudo /usr/local/bin/debsecan-filtered --test` against a host with known CVEs. Compare output before/after.
4. **Lint it.** Run [`shellcheck`](https://www.shellcheck.net/) locally:
   ```bash
   shellcheck setup-cve-alerts.sh debsecan-filtered uninstall.sh
   ```
...
- **Comments:** explain *why*, not *what*. The Phase 1–9 banner comments in `debsecan-filtered` are a good model.
...
```
feat: add --quiet flag to debsecan-filtered
